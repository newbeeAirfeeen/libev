...
= item ev_set_syserr_cb(void (*cb)(const char *msg) throw())
          ...
          error(such
                        as failed select,
                poll, epoll_wait)
                  .The message is a printable string
                          indicating the system call or
  subsystem causing the problem.If this callback is set,
        then libev will expect it to remedy the situation, no matter what, when it returns.That is, libev will generally retry the requested operation, or, if the condition doesn't go away, do bad stuff (such as abort).
   ...
   adns_prepare_cb (struct ev_loop *loop, ev_prepare *w, int revents)
   {
    ...
    for (int i = 0; i < nfd; ++i) {
        ev_io_init(iow + i, io_cb, fds[i].fd,
                   ((fds[i].events & POLLIN ? EV_READ : 0) | (fds[i].events & POLLOUT ? EV_WRITE : 0)));
...
watchers in the constructor.
   ...
   class myclass
   {
    ...
    void io_cb(ev::io & w, int revents);
    ev::io io2;
    void io2_cb(ev::io & w, int revents);
    ev::idle idle;
